...
const UserPanel: React.FC = () => {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      setPasswordMessage(data.message || messages.userPanel.password.successFallback);
      if (data.message.includes("successfully")) {
        setNewPassword("");
      }
    } catch (error: unknown) {
  ...
  const handleAddGithubUsername = async () => {
    ...
    try {
      ...
      setGithubMessage(data.message || messages.userPanel.github.successFallback);
      if (data.message.includes("successfully")) {
  ...
  return (
    <div className="min-h-screen">
      <TopNavBar title={messages.userPanel.pageTitle} showBackButton={true} showUserInfo={true} />
        ...
        <SectionCard title={messages.userPanel.sectionTitle}>
          <div className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <Button onClick={handleSubmitAll}>{messages.userPanel.submitChanges}</Button>
              <Button
                variant="secondary"
                onClick={() => {
                  setNewEmail("");
