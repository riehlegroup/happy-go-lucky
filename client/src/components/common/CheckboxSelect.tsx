...
const CheckboxSelect = ({
  ...
  return (
    <div>
      ...
      >
        {children}
        <svg className="ms-3 size-2.5" fill="none" viewBox="0 0 10 6">
          <path
            stroke="currentColor"
            strokeLinecap="round"
          ...
          <ul className="space-y-3 p-3 text-sm text-gray-700">
            <li className="flex items-center">
              <input
                ...
                onChange={(e) =>
                  e.target.checked ? setActive(options) : setActive([])
                }
              ...
              return (
                <li key={id} className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      e.target.checked
                        ? setActive(active.concat(option))
                        : setActive(active.filter((a) => a != option))
                    }
                    className="size-4 rounded border-gray-300 bg-gray-100 text-blue-600"
                  />
                  <label className="ms-2 text-sm font-medium text-gray-900">
