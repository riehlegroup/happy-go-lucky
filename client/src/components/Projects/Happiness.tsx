...
const Happiness: React.FC = (): React.ReactNode => {
  ...
  const [userCurrentHappiness, setUserCurrentHappiness] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    const fetchCourseId = async () => {
      ...
      try {
        const course = await ApiClient.getInstance().get<{
          courseId: number;
          courseName: string;
    ...
    const fetchNextSubmission = async () => {
      ...
      try {
        const submission = await ApiClient.getInstance().get<{
          id: number;
          submissionDate: string;
        }>("/courseProject/availableSubmissions", { projectName: projectName });
  ...
  const handleHappinessSubmit = async (ratingValue: number) => {
    if (!projectName || !user?.email || !nextSubmission) {
      alert(messages.projects.happiness.missingInfoAlert);
      return;
    }
  ...
  const fetchHappinessData = async () => {
    try {
      ...
      >("/courseProject/happiness", { projectName: projectName ?? "" });
      ...
      if (happinessData.length > 0) {
        const userEntry = happinessData.find(
          (entry) =>
            ...
            entry.submissionDateId === nextSubmission.id,
        );
  ...
  if (allSubmissionDates.length > 0) {
    ...
    const uniqueTimestamps = [
      ...new Set(happinessData.map((d) => d.submissionDate)),
    ];
  ...
  const formattedData: {
    [sprintLabel: string]: {
      sprintLabel: string;
      [userEmail: string]: number | string | undefined;
    };
  } = {};
  const now = new Date();
  ...
  const nextSubmissionDate =
    futureDates.length > 0 ? futureDates[0].dateKey : null;
  ...
  happinessData.forEach((data) => {
    ...
    if (!sprintNumber) {
      console.warn(
        `No sprint number found for date ${dateKey} (timestamp: ${data.submissionDate})`,
      );
      return;
    }
  ...
  return (
    <div className="min-h-screen">
      <TopNavBar
        title={messages.projects.happiness.pageTitle}
        showBackButton={true}
        showUserInfo={true}
      />
      <div className="mx-auto max-w-6xl space-y-4 p-4 pt-16">
        <Tabs defaultValue="User" className="w-full">
          <TabsList className="inline-flex h-auto gap-1 bg-slate-100 p-2">
            ...
            >
              {messages.projects.happiness.tabs.user}
            </TabsTrigger>
            <TabsTrigger
              value="Display"
              className="data-[state=active]:bg-white data-[state=active]:shadow"
            >
              {messages.projects.happiness.tabs.display}
            </TabsTrigger>
          </TabsList>
          <TabsContent value="User">
            <SectionCard
              title={messages.projects.happiness.submit.sectionTitle}
            >
              {!nextSubmission ? (
                <div className="rounded-md bg-red-50 p-4 text-center text-sm text-red-700">
                  {messages.projects.happiness.submit.noSubmissionWindow}
                </div>
              ) : (
                <>
                  <div className="space-y-6">
                    <div className="text-sm text-slate-600">
                      {messages.projects.happiness.submit.submitBeforeLabel}{" "}
                      <span className="font-semibold">
                        {moment(nextSubmission.submissionDate).format(
                          "YYYY-MM-DD HH:mm:ss",
                        )}
                      </span>
                    </div>
                    <div className="space-y-6">
                      <div className="text-lg font-semibold text-slate-900">
                        {messages.projects.happiness.submit.question}
                      </div>
                      ...
                      {showSuccess && (
                        <div className="rounded-md bg-green-50 p-3 text-center text-sm font-medium text-green-700">
                          {messages.projects.happiness.submit.success}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <TabsContent value="Display">
            <SectionCard
              title={messages.projects.happiness.display.sectionTitle(
                projectName,
              )}
            >
