...
export function checkAdmin(db: Database) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) {
      res.status(401).json({ success: false, message: messages.middleware.authenticationRequired });
      return;
    }
    ...
    try {
      ...
      if (!user) {
        res.status(404).json({ success: false, message: messages.middleware.userNotFound });
        return;
      }
      ...
      res.status(401).json({ success: false, message: messages.middleware.invalidToken });
      return;
    }
