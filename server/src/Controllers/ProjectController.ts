...
export class ProjectController implements IAppController {
  ...
  async getProjects(req: Request, res: Response): Promise<void> {
    ...
    if (!courseName) {
      res.status(400).json({ message: Messages.project.courseIdRequired });
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes("Unknown Course Name!")) {
          res.status(404).json({ message: Messages.course.courseNotFound });
          return;
        }
        throw error;
      }
      ...
      res
        .status(500)
        .json({ message: Messages.project.failedToRetrieveProjectsForCourse(courseName.toString()), error });
    }
  }
  ...
  async editProject(req: Request, res: Response): Promise<void> {
    ...
    if (!newCourseName || !newProjectName) {
      res.status(400).json({ message:  Messages.project.projectEditMissingFields });
      return;
    }
  ...
  async getCourseForProject(req: Request, res: Response): Promise<void> {
    ...
    if (!projectName) {
      res.status(400).json({ message: Messages.project.projectNameRequired });
      return;
    }
    ...
    try {
      ...
      if (course) {
        ...
        res.status(404).json({ message: Messages.project.courseNotFoundForProject });
      }
    } catch (error) {
  ...
  async getRoleForProject(req: Request, res: Response): Promise<void> {
    ...
    if (!projectName) {
      res.status(400).json({ message: Messages.project.projectNameAndUserEmailRequired });
      return;
    }
    ...
    try {
      ...
      res.status(500).json({ message: Messages.project.failedToRetrieveProjectRole, error });
    }
  }
  ...
  async joinProject(req: Request, res: Response): Promise<void> {
    ...
    if (!memberRole) {
      res.status(400).json({ message: Messages.project.roleRequired });
      return;
    }
  ...
  async getUserProjects(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.status(500).json({ message: Messages.project.failedToRetrieveUserProjects, error });
    }
  }
  ...
  async getEnrolledCourses(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.status(500).json({ message: Messages.project.failedToRetrieveCoursesOfUser, error });
    }
  }
  ...
  async saveHappinessMetric(req: Request, res: Response): Promise<void> {
    ...
    if (!submissionDateId || typeof submissionDateId !== 'number') {
      res.status(400).json({ message: Messages.project.submissionDateIdRequiredNumber });
      return;
    }
  ...
  async getProjectHappinessMetrics(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.status(500).json({ message: Messages.project.failedToFetchHappinessData, error });
    }
  }
  ...
  async getAvailableSubmissions(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!schedule) {
        res.status(404).json({ message: Messages.project.noScheduleFoundForCourse });
        return;
      }
      ...
      if (!nextSubmission) {
        res.status(404).json({ message: Messages.project.noFutureSubmissionDatesAvailable });
        return;
      }
      ...
      res.status(500).json({ message: Messages.project.failedToFetchAvailableSubmissions, error });
    }
  }
  ...
  async sendStandupEmails(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      await this.emailService.sendEmail(
        recipientEmails,
        Messages.email.standupEmailSubject(projectName),
        Messages.email.standupEmailBody(userName, doneText, plansText, challengesText)
      );
