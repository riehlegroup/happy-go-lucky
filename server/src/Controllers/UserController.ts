...
export class UserController implements IAppController {
  ...
  async getUsersByStatus(req: Request, res: Response): Promise<void> {
    ...
    try {
      const user = await this.db.all("SELECT * FROM users WHERE status = ?", [
        status,
      ]);
      if (user) {
        res.json(user);
  ...
  async updateUserStatus(req: Request, res: Response): Promise<void> {
    ...
    if (!userEmail || !status) {
      res
        .status(400)
        .json({ message: messages.user.pleaseProvideEmailAndStatus });
      return;
    }
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: messages.user.userStatusUpdatedSuccessfully });
    } catch (error) {
      console.error("Error during updating user status:", error);
      res
        .status(500)
        .json({ message: messages.user.failedToUpdateUserStatus, error });
    }
  }
  ...
  async updateAllConfirmedUsers(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (result.changes === 0) {
        res
          .status(404)
          .json({ message: messages.user.noConfirmedUsersFoundToUpdate });
        return;
      }

      res
        .status(200)
        .json({ message: messages.user.allConfirmedUsersUpdatedTo(status) });
    } catch (error) {
      console.error("Error updating confirmed users:", error);
      res
        .status(500)
        .json({ message: messages.user.failedToUpdateConfirmedUsers });
    }
  }
  ...
  async changeEmail(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      await this.db.run(`UPDATE users SET email = ? WHERE id = ?`, [
        newEmail,
        userId,
      ]);
      ...
      res
        .status(500)
        .json({ message: messages.user.failedToUpdateEmail, error });
    }
  }
  ...
  async changePassword(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: messages.user.passwordUpdatedSuccessfully });
    } catch (error) {
      console.error("Error updating password:", error);
      res
        .status(500)
        .json({ message: messages.user.failedToUpdatePassword, error });
    }
  }
  ...
  async getUserProjectURL(req: Request, res: Response): Promise<void> {
    ...
    if (!userEmail || !projectName) {
      res
        .status(400)
        .json({ message: messages.user.userEmailAndProjectNameMandatory });
      return;
    }
  ...
  async setUserGitHubUsername(req: Request, res: Response): Promise<void> {
    ...
    if (!newGithubUsername) {
      res
        .status(400)
        .json({ message: messages.user.pleaseFillInGithubUsername });
      return;
    }
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: messages.user.githubUsernameAddedSuccessfully });
    } catch (error) {
      console.error("Error adding GitHub username:", error);
      res
        .status(500)
        .json({ message: messages.user.failedToAddGithubUsername, error });
    }
  }
  ...
  async getUserGitHubUsername(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const githubUsernameObj = await this.db.get(
        `SELECT githubUsername FROM users WHERE id = ?`,
        [userId],
      );
      ...
      res
        .status(500)
        .json({ message: messages.user.failedToFetchGithubUsername, error });
    }
  }
  ...
  async getUserRole(req: Request, res: Response): Promise<void> {
    ...
    try {
      const user = await this.db.get(
        "SELECT userRole FROM users WHERE email = ?",
        [userEmail],
      );
      if (user) {
        res.json(user);
      ...
      res
        .status(500)
        .json({ message: messages.user.failedToRetrieveUserRole, error });
    }
  }
  ...
  async updateUserRole(req: Request, res: Response): Promise<void> {
    ...
    if (!email || !role) {
      res
        .status(400)
        .json({ message: messages.user.pleaseProvideEmailAndRole });
      return;
    }

    try {
      await this.db.run("UPDATE users SET userRole = ? WHERE email = ?", [
        role,
        email,
      ]);
      res
        .status(200)
        .json({ message: messages.user.userRoleUpdatedSuccessfully });
    } catch (error) {
      console.error("Error during updating user role:", error);
      res
        .status(500)
        .json({ message: messages.user.failedToUpdateUserRole, error });
    }
  }
