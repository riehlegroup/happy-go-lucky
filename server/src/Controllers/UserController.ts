...
export class UserController implements IAppController {
  ...
  async getUsers(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ message: Messages.user.failedToRetrieveUserStatus, error });
    }
  }
  ...
  async updateUserStatus(req: Request, res: Response): Promise<void> {
    ...
    if (!userEmail || !status) {
      res.status(400).json({ message: Messages.user.emailAndStatusRequired });
      return;
    }
  ...
  async updateAllConfirmedUsers(req: Request, res: Response): Promise<void> {
    ...
    if (!status) {
      res.status(400).json({ message: Messages.user.statusRequired });
      return;
    }
  ...
  async changeEmail(req: Request, res: Response): Promise<void> {
    ...
    if (!newEmail) {
      res.status(400).json({ message: Messages.user.newEmailRequired});
      return;
  ...
  async changePassword(req: Request, res: Response): Promise<void> {
    ...
    if (!password) {
      res.status(400).json({ message: Messages.user.newPasswordRequired});
      return;
    } else if (password.length < 8) {
      res.status(400).json({ message: Messages.user.passwordMinLength });
      return;
    }
  ...
  async setUserGitHubUsername(req: Request, res: Response): Promise<void> {
    ...
    if (!newGithubUsername) {
      res.status(400).json({ message: Messages.user.githubUsernameRequired});
      return;
    }
  ...
  async getUserGitHubUsername(req: Request, res: Response): Promise<void> {
    ...
    if (!userEmail) {
      res.status(400).json({ message: Messages.user.userEmailMandatory });
      return;
    }
  ...
  async updateUserRole(req: Request, res: Response): Promise<void> {
    ...
    if (!email || !role) {
      res.status(400).json({ message: Messages.user.emailAndRoleRequired});
      return;
    }
  ...
  private async sendSuspendedEmail(email: string): Promise<void> {
    await this.emailService.sendEmail(
      email,
      Messages.email.accountSuspendedSubject,
      Messages.email.accountSuspendedBody,
    );
  }
  ...
  private async sendRemovedEmail(email: string): Promise<void> {
    await this.emailService.sendEmail(
      email,
      Messages.email.accountRemovedSubject,
      Messages.email.accountRemovedBody,
    );
  }
}
