...
import { requireAdmin, requireAuth, requireCompetitionActive, requireCompetitionExists, requireCourseMember } from "../middleware/auth.middleware";
...
export function createCompetitionRouter(db: Database, authRepo: AuthentificationRepo, competitionRepo: CompetitionRepo, datasetRepo: DatasetRepo, evaluationConfigRepo: EvaluationConfigRepo, submissionRepo: SubmissionRepo, competitionService: CompetitionService): Router {
	...
	const datasetService = new DatasetService(datasetRepo, evaluationConfigRepo);
